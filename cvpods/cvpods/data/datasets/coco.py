...
class COCODataset(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=None, is_train=True):
        ...
        self.annoted_imgs = None

        if is_train:
            percent = cfg.DATASETS.PERCENT
    ...
    """)
        ...
        if self.annoted_imgs is not None:
            img_ids = set(img_ids) & set(self.annoted_imgs)
            img_ids = sorted(img_ids)
            print('filtered img ids num .. ', len(img_ids))
            # img_ids =
        ...
        for (img_dict, anno_dict_list) in imgs_anns:
            ...
            for anno in anno_dict_list:
                ...
                pseudo_bbox = anno.get("pseudo-bbox", None)
                if pseudo_bbox:
                    # print(obj["bbox"], pseudo_bbox)
                    #override the original gt bbox
                    obj["bbox"] = pseudo_bbox
                # else:
                    # coco_api.imgToAnns[img_id]
                    # print('pseudo-bbox not exist .. ', anno, flush=True)
