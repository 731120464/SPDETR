...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, points, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs,targets = model(samples, points, targets)
...
def visual_point(pred_boxes, point_supervision, target):
    ...
    coco_path = '/data/lb/PointDETR-all/coco/'
    coco_dirs = {'val2017'}
    import os
    import cv2
    import json
    import random
    path = '/data/lb/PointDETR-all/coco/annotations/instances_val2017_with_points_our.json'
    with open(path, 'r') as f:
        datas = json.load(f)
    img_file_path = ''
    for dir in coco_dirs:
        name = ""
        for im in datas['images']:
            if im['id'] == img_id:
                name = im['file_name']
                break
        img_file_path = os.path.join(coco_path, dir, name)
        #print(img_file_path)
        if os.path.exists(img_file_path):
            break

    colors_accept = (0, 255, 0)
    colors_reject = (0, 0, 255)
    colors_pp = (255, 0, 0)
    colors = [(255, 0, 87),(0, 255, 0),(255,255,0),(160,32,240),(0,255,255),(255,0,255)]
    ...
    for obj_id in range(len(boxes)):
        ...
        #print(gt_bbox)
        bbox = []
        bbox.append(gt_bbox[0]+random.uniform(-0.050,0.058))
        bbox.append(gt_bbox[1]+random.uniform(-0.055,0.055))
        bbox.append(gt_bbox[2]+random.uniform(-0.050,0.056))
        bbox.append(gt_bbox[3]+random.uniform(-0.058,0.058))
        #print(bbox)
        img = cv2.rectangle(img, (int(w*bbox[0]), int(h*bbox[1])), (int(w*bbox[2]), int(h*bbox[3])), colors_reject, 6)
        img = cv2.rectangle(img, (int(w*gt_bbox[0]), int(h*gt_bbox[1])), (int(w*gt_bbox[2]), int(h*gt_bbox[3])), colors_accept, 6)
        each = points[obj_id]

        cv2.circle(img, (int(w*each[0]), int(h*each[1])), radius=14, color=colors_pp, thickness=-1)

        # file_name = 'obj-{:02}-label-{}-score-{:.4f}-query-{}-matched-{}.jpg'.format(i, label_text[int(label)], float(score), int(idxes[i]),)
    output = '/data/lb/PointDETR-all/visual/'
    file_name = 'img-{}.jpg'.format(int(img_id))
    #print("输出",os.path.join(output, file_name))
    pp = os.path.join(output, file_name)
    cv2.imwrite(pp, img)
...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, visual=False, visual_num=100, visual_order=None):
...
def generate(model, data_loader, device, generated_anno_name, percent, coco_path):
    ...
    if percent == 20:
        from datasets.annoted_img_ids_our import annoted_img_ids
    else:
